...
NSString *const KS3iOSSDKServiceErrorDomain =
    @"com.ks3yun.iossdk.ServiceErrorDomain";
NSString *const KS3iOSSDKClientErrorDomain =
    @"com.ks3yun.iossdk.ClientErrorDomain";
  ...
  if ([exception isKindOfClass:[KS3ClientException class]]) {
    ...
    if (clientException.error != nil) {
      ...
      NSDictionary *userInfo = [NSDictionary
          dictionaryWithObjectsAndKeys:clientException.message, @"message",
                                       clientException, @"exception", nil];

      error =
          [NSError errorWithDomain:clientErrorDomain code:-1 userInfo:userInfo];
    }
  }
