...
didReceiveResponse:(NSURLResponse *)response {
  ...
  // setting response header to use it in shouldRetry method of
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
  ...
  if ([self.request.delegate
          respondsToSelector:@selector(request:didReceiveData:)]) {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  ...
  if (_request.delegate &&
      [_request.delegate
          respondsToSelector:@selector(request:didCompleteWithResponse:)]) {
...
- (void)connection:(NSURLConnection *)connection
  ...
  //        valueForKey:key] description]);
  //    }
  if ([self.request.delegate
          respondsToSelector:@selector(request:didFailWithError:)]) {
...
- (void)connection:(NSURLConnection *)connection
  ...
  if ([self.request.delegate
          respondsToSelector:@selector(request:
                                               didSendData:
                                         totalBytesWritten:
...
// The request gets re-written with a GET http method. This is to set the method
// back to
// the appropriate method if necessary
- (NSURLRequest *)connection:(NSURLConnection *)connection
             willSendRequest:(NSURLRequest *)proposedRequest
