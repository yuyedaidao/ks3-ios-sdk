...
- (KS3URLRequest *)configureURLRequest {
  ...
  if (nil != _xkssMetaDataDirective) {
    [self.urlRequest setValue:_xkssMetaDataDirective
           forHTTPHeaderField:@"x-kss-metadata-directive"];
  }
  if (nil != _xkssCopySourceIfMatch) {
    [self.urlRequest setValue:_xkssCopySourceIfMatch
           forHTTPHeaderField:@"x-kss-copy-source-if-match"];
  }
  if (nil != _xkssCopySourceIfNoneMatch) {
    [self.urlRequest setValue:_xkssCopySourceIfNoneMatch
           forHTTPHeaderField:@"x-kss-copy-source-if-none-match"];
  }
  if (nil != _xkssCopySourceIfUnmodifiedSince) {
    [self.urlRequest setValue:_xkssCopySourceIfUnmodifiedSince
           forHTTPHeaderField:@"x-kss-copy-source-if-unmodified-since"];
  }
  if (nil != _xkssCopySourceIfModifiedSince) {
    [self.urlRequest setValue:_xkssCopySourceIfModifiedSince
           forHTTPHeaderField:@"x-kss-copy-source-if-modified-since"];
  }
  if (nil != _xkssMeta) {
    [self.urlRequest setValue:_xkssMeta forHTTPHeaderField:@"x-kss-meta-"];
  }
  if (nil != _xkssStorageClass) {
    [self.urlRequest setValue:_xkssStorageClass
           forHTTPHeaderField:@"x-kss-storage-class"];
  }
  if (nil != _xkssWebsiteRedirectLocation) {
    [self.urlRequest setValue:_xkssWebsiteRedirectLocation
           forHTTPHeaderField:@"x-kss-website-redirect-location"];
  }

  // **** encryption header
  if (nil != _xkssServerSideEncryption) {
    [self.urlRequest setValue:_xkssServerSideEncryption
           forHTTPHeaderField:@"x-kss-server-side-encryption"];
  }
  if (nil != _xkssServerSideEncryptionCustomerKey) {
    [self.urlRequest setValue:_xkssServerSideEncryptionCustomerKey
           forHTTPHeaderField:@"x-kss-server-side-encryption-customer-key"];
  }
  if (nil != _xkssServerSideEncryptionCustomerAlgorithm) {
    [self.urlRequest
                  setValue:_xkssServerSideEncryptionCustomerAlgorithm
        forHTTPHeaderField:@"x-kss-server-side-encryption-customer-algorithm"];
  }
  if (nil != _xkssServerSideEncryptionCustomerKeyMD5) {
    [self.urlRequest setValue:_xkssServerSideEncryptionCustomerKeyMD5
           forHTTPHeaderField:@"x-kss-server-side-encryption-customer-key-MD5"];
  }

  if (nil != _xkssCopySourceServerSideEncryptionCustomerKey) {
    [self.urlRequest setValue:_xkssCopySourceServerSideEncryptionCustomerKey
           forHTTPHeaderField:
               @"x-kss-copy-source-server-side-encryption-customer-key"];
  }
  if (nil != _xkssCopySourceServerSideEncryptionCustomerAlgorithm) {
    [self.urlRequest
                  setValue:_xkssCopySourceServerSideEncryptionCustomerAlgorithm
        forHTTPHeaderField:
            @"x-kss-copy-source-server-side-encryption-customer-algorithm"];
  }
  if (nil != _xkssCopySourceServerSideEncryptionCustomerMD5) {
    [self.urlRequest setValue:_xkssCopySourceServerSideEncryptionCustomerMD5
           forHTTPHeaderField:
