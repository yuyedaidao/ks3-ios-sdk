...
- (void)start {
  if (!url) {
    if (delegate && [delegate respondsToSelector:@selector(downloadFaild:
                                                        didFailWithError:)]) {
      NSError *error = [NSError errorWithDomain:@"Url can not be nil!"
                                           code:110
                                       userInfo:nil];
  ...
  //        if ([fileName length] > 32) fileName = [fileName
  //        substringFromIndex:[fileName length]-32];
  //    }
  _isStop = NO;
  ...
  temporaryPath =
      [filePath stringByAppendingPathComponent:deletingPathExtension];
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:destinationPath]) {
    if (overwrite) {
      ...
      if (delegate &&
          [delegate
              respondsToSelector:@selector(downloadProgressChange:progress:)]) {
        [delegate downloadProgressChange:self progress:1.0];
      }
      if (delegate &&
          [delegate respondsToSelector:@selector(downloadFinished:filePath:)]) {
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath:temporaryPath]) {
    ...
    if (!createSucces) {
      if (delegate && [delegate respondsToSelector:@selector(downloadFaild:
                                                          didFailWithError:)]) {
        NSError *error =
            [NSError errorWithDomain:@"Temporary File can not be create!"
                                code:111
                            userInfo:nil];
        [delegate downloadFaild:self didFailWithError:error];
      }
  ...
  if (customBucketDomain != nil) {
    ...
    strHost = [NSString
        stringWithFormat:@"%@://%@.%@/%@", [ks3Client requestProtocol],
                         _bucketName, [ks3Client getBucketDomain], _key];
  }

  NSString *strAuthorization = @"";
  if (_credentials.accessKey != nil && _credentials.secretKey != nil) {
    strAuthorization =
        [KS3AuthUtils strAuthorizationWithHTTPVerb:_credentials.accessKey
  ...
  if (_credentials == nil) {
    ...
    //        NSDictionary *dicParams = [NSDictionary
  ...
  while (!_isFinished) {
    ...
    [[NSRunLoop currentRunLoop] runMode:KSYS3DefaultRunLoopMode
...
didReceiveResponse:(NSURLResponse *)response {
  ...
  if (delegate &&
      [delegate respondsToSelector:@selector(downloadBegin:
