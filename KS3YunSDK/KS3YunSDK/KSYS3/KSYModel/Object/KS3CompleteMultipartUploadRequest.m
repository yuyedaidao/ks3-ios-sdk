...
- (void)setCompleteRequest {
      ...
      for (NSString *strKey in _callbackParams.allKeys) {
        if (strKey.length >= 4 &&
          ...
          NSLog(@"The header with field: \"%@\" and value: \"%@\" is not "
                @"correct, this header will be ingored",
  ...
  [self setKSYResource:[NSString stringWithFormat:@"%@/%@?%@=%@",
                                                  self.kSYResource, _key,
                                                  kKS3QueryParamUploadId,
                                                  self.uploadId]];
...
- (NSURLRequest *)configureURLRequest {
  ...
  [self.urlRequest setValue:@"text/xml"
...
- (NSData *)requestBody {
  ...
  NSArray *keys =
      [[self.parts allKeys] sortedArrayUsingComparator:comparePartNumbers];
  for (NSNumber *partNumber in keys) {
    [xml
        appendFormat:@"<Part><PartNumber>%d</PartNumber><ETag>%@</ETag></Part>",
                     [partNumber intValue],
                     [self.parts objectForKey:partNumber]];
  }
