...
+ (KS3ClientException *)validateBucketName:(NSString *)theBucketName {
  ...
  if ([theBucketName length] < 3 || [theBucketName length] > 255) {
    return [KS3ClientException
        exceptionWithMessage:
            @"Bucket name should be between 3 and 255 characters in length."];
  }
  if ([KS3SDKUtil isVaildBucketName:theBucketName]) {
    return [KS3ClientException
        exceptionWithMessage:
            @"Bucket name 只能包含小写英文字母（a-z），数字，点（.），中线."];
  }

  if ([theBucketName hasPrefix:@"kss"]) {
    return [KS3ClientException
        exceptionWithMessage:@"Bucket name should not start with a 'kss'."];
  }
  if ([theBucketName hasPrefix:@"."]) {
    return [KS3ClientException
        exceptionWithMessage:@"Bucket name should not start with a '.'."];
  }
  if ([theBucketName hasPrefix:@"-"]) {
    return [KS3ClientException
        exceptionWithMessage:@"Bucket name should not start with a '-'."];
  ...
  if ([KS3SDKUtil isIpString:theBucketName]) {
    return [KS3ClientException
        exceptionWithMessage:@"Bucket name should not be a IP"];
  }

  //    if ( [KS3BucketNameUtilities contains:theBucketName searchString:@".."])
  //    {
  //        return [KS3ClientException exceptionWithMessage : @"Bucket name
  //        should not contain two adjacent periods."];
  //    }
  //
  ...
  //        {
  //        return [KS3ClientException exceptionWithMessage : @"Bucket name
  //        should not contain dashes next to periods."];
  //    }
  //
  //    if ( [[theBucketName lowercaseString] isEqualToString:theBucketName] ==
  //    NO) {
  //        return [KS3ClientException exceptionWithMessage : @"Bucket name
  //        should not contain upper case characters."];
  //    }
